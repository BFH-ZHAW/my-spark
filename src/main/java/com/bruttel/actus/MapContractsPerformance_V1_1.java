...
public class MapContractsPerformance_V1_1 {
	...
	public static void main(String[] args) {
		...
		String className = "com.bruttel.actus.MapContractsPerformance_V1_1";
		...
		JavaRDD<String> riskFactor = sparkSession.read().textFile(riskfactorsPath).javaRDD().cache(); 
		JavaPairRDD<String, String[]> riskFactorRDD = riskFactor
				.mapToPair(temp -> new Tuple2<String, String[]>(temp.split(";")[0], temp.split(";"))).cache();
		...
		JavaRDD<String> contractFile = sparkSession.read().textFile(contractsPath).javaRDD().cache(); 
			// Log Schreiben
		...
		for (int i=2; i <= 3; i++) {
					...
					contractFile = sparkSession.read().textFile(contractsPath).javaRDD().cache(); 
					// Log Schreiben
		...
		JavaPairRDD<String, String[]> contractFileRDD = contractFile
				.mapToPair(temp -> new Tuple2<String, String[]>(temp.split(";")[40], temp.split(";"))).cache();
		
		
		// JOIN TEIL
			//Perfomrance Messung
			long startJoin = System.currentTimeMillis();
		JavaPairRDD<String, Tuple2<String[], String[]>> contractsAndRisk = contractFileRDD.join(riskFactorRDD).cache();
		// Log Schreiben
			new WriteLog(logFile, className.concat("_Join"), output, ram, String.valueOf(contractsAndRisk.count()), riskfactors, knoten, "1", startJoin, System.currentTimeMillis());
				for (int i=2; i <= 3; i++) {
							...
							contractsAndRisk = contractFileRDD.join(riskFactorRDD).cache();
							// Log Schreiben
		...
		JavaRDD<Row> events = contractsAndRisk.values().flatMap(new MapFunction_V2(_t0)).cache();
		// Log Schreiben
					new WriteLog(logFile, className.concat("_Flatmap"), output, ram, String.valueOf(events.count()), riskfactors, knoten, "1", startFlatmap, System.currentTimeMillis());
						for (int i=2; i <= 3; i++) {
									...
									events = contractsAndRisk.values().flatMap(new MapFunction_V2(_t0)).cache();
									// Log Schreiben
