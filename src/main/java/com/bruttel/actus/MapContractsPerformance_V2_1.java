...
public class MapContractsPerformance_V2_1 {
	...
	public static void main(String[] args) {
		...
		String className = "com.bruttel.actus.MapContractsPerformance_V2_1";
		...
		Dataset<Row> timespecsFile = sparkSession.read().option("header", false).csv(timespecsPath).cache();
		...
		Dataset<Row> riskfactorsFile = sparkSession.read().option("header", true).option("sep",";").csv(riskfactorsPath).cache();    
		...
		// IMPORT TEIL
			...
			Dataset<Row> contractsFile = sparkSession.read().option("header", true).option("sep",";").csv(contractsPath).cache();
		...
		for (int i=2; i <= 3; i++) {
					...
					contractsFile = sparkSession.read().option("header", true).option("sep",";").csv(contractsPath).cache();
				   	contractsAmount = contractsFile.count();					// Log Schreiben
					new WriteLog(logFile, className.concat("_Read"), output, ram, String.valueOf(contractsFile.count()), riskfactors, knoten,  String.valueOf(i), startRead_i, System.currentTimeMillis());
			} 
		...
		// JOIN TEIL
		    ...
		    Dataset<Row> filesJoin = contractsFile.join(riskfactorsFile, riskfactorsFile.col("MarketObjectCode").equalTo(contractsFile.col("marketObjectCodeRateReset"))).cache();
		// Log Schreiben
			new WriteLog(logFile, className.concat("_Join"), output, ram, String.valueOf(filesJoin.count()), riskfactors, knoten, "1", startJoin, System.currentTimeMillis());
				for (int i=2; i <= 3; i++) {
							...
							long startJoin_i = System.currentTimeMillis();
						     filesJoin = contractsFile.join(riskfactorsFile, riskfactorsFile.col("MarketObjectCode").equalTo(contractsFile.col("marketObjectCodeRateReset"))).cache();
							// Log Schreiben
							new WriteLog(logFile, className.concat("_Join"), output, ram, String.valueOf(filesJoin.count()), riskfactors, knoten, String.valueOf(i), startJoin_i, System.currentTimeMillis());
					} 
				
		//FLATMAP TEIL
			//Perfomrance Messung
			long startFlatmap = System.currentTimeMillis();
			JavaRDD<Row> events = filesJoin.javaRDD().flatMap(new MapFunction_V3(_t0)).cache();
			// Log Schreiben
					new WriteLog(logFile, className.concat("_Flatmap"), output, ram, String.valueOf(events.count()), riskfactors, knoten, "1", startFlatmap, System.currentTimeMillis());
						for (int i=2; i <= 3; i++) {
									...
									events = filesJoin.javaRDD().flatMap(new MapFunction_V3(_t0)).cache();
									// Log Schreiben
		...
		new WriteLog(logFile, className.concat("_Datframe"), output, ram, String.valueOf(cachedEvents.count()), riskfactors, knoten, "1", startDataFrame, System.currentTimeMillis());
			for (int i=2; i <= 3; i++) {
						...
						cachedEvents = sparkSession.createDataFrame(events, eventsSchema).cache();
						// Log Schreiben
