...
public class CallculateNominalValue_V0_0 {
	...
	public static void main(String[] args) {
		...
		String className = "com.bruttel.actus.CallculateNominalValue_V0_0";
		...
		String sqlquery = "SELECT riskScenario, "
						+ "		portfolio, "
						+ "		sum(nominal) / 1000000 as NominalInMio "
						+ "FROM events "
						+ "WHERE type = 'AD0' "
						+ "GROUP BY riskScenario, portfolio";
		
		//Execute SQL
		Dataset<Row> nominalValues = sparkSession.sql(sqlquery).cache();
		...
		//Register as table to use sql
		nominalValues.createOrReplaceTempView("nominalValues");

		//SQL query with no sucess to get no files back
		String sqlquery2 = "SELECT portfolio, "
						+ "		mean(NominalInMio) as MeanNominalInMio, "
						+ "		max(NominalInMio) as MaxNominalinMio, "
						+ "		min(NominalInMio) as MinNominalinMio "
						+ "FROM nominalValues "
						+ "GROUP BY  portfolio "
						+ "ORDER BY portfolio";
		
		//Execute SQL2
		Dataset<Row> nominalValuesMean = sparkSession.sql(sqlquery2).cache();
		...
		// stop spark Session
		nominalValues.unpersist(true);
		nominalValuesMean.unpersist(true);
		sparkSession.sqlContext().dropTempTable("events");
		sparkSession.sqlContext().dropTempTable("nominalValues");
		sparkSession.stop();
