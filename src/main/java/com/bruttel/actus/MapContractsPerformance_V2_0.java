...
public class MapContractsPerformance_V2_0 {
	...
	public static void main(String[] args) {
		...
		String className = "com.bruttel.actus.MapContractsPerformance_V2_0";
		...
		for (int i=2; i <= 3; i++) {
					...
					contractsFile = sparkSession.read().option("header", true).option("sep",";").csv(contractsPath);
			...
			new WriteLog(logFile, className.concat("_Join"), output, ram, String.valueOf(filesJoin.count()), riskfactors, knoten, "1", startJoin, System.currentTimeMillis());
				for (int i=2; i <= 3; i++) {
							...
							long startJoin_i = System.currentTimeMillis();
						     filesJoin = contractsFile.join(riskfactorsFile, riskfactorsFile.col("MarketObjectCode").equalTo(contractsFile.col("marketObjectCodeRateReset")));
							// Log Schreiben
			...
			// Log Schreiben
					new WriteLog(logFile, className.concat("_Flatmap"), output, ram, String.valueOf(events.count()), riskfactors, knoten, "1", startFlatmap, System.currentTimeMillis());
						for (int i=2; i <= 3; i++) {
									...
									events = filesJoin.javaRDD().flatMap(new MapFunction_V3(_t0));
									// Log Schreiben
